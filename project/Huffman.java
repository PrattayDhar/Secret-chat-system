...
public class Huffman {
	...
	public PriorityQueue<node> pq;  // for MinHeap
	public int counter;  // Unique id assigned to each node
	...
	public void buildCodeRecursion(node n, String code){
		if (n != null){
			if (! isLeaf(n)){  // n = internal node
				buildCodeRecursion(n.left, code + '0');
		buildCodeRecursion(n.right, code + '1');
			}
	else{  // n = Leaf node		hmapCode.put(n.ch, code); // for {character:code}
